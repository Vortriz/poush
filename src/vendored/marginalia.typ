...
#let note-numbering(
    ...
    let index = if repeat and markers.len() > 0 {
        calc.rem(i - 1, markers.len())
    } else { i - 1 }
    ...
    show ref: it => {
        if (
            ...
            if it.element.children.first().value == "_marginalia_note" {
                ...
                let dest = query(
                    selector(<_marginalia_note>).after(it.element.location()),
                )
                assert(
                    dest.len() > 0,
                    message: "Could not find referenced note",
                )
                link(dest.first().location(), dest.first().value.anchor)
            } else if (
                it.element.children.first().value == "_marginalia_notefigure"
            ) {
                let dest = query(
                    selector(<_marginalia_notefigure_meta>).after(
                        it.element.location(),
                    ),
                )
                assert(
                    dest.len() > 0,
                    message: "Could not find referenced notefigure",
                )
                if it.has("form") {
                    std.ref(
                        dest.first().value.label,
                        form: it.form,
...
#let show-frame(
    ...
    set page(
        background: context {
            ...
            let ascent = (
                page.header-ascent.ratio * topm + page.header-ascent.length
            )
            place(top, dy: topm, line(length: 100%, stroke: stroke))
            if header {
                place(top, dy: topm - ascent, line(
                    length: 100%,
                    stroke: stroke,
                ))
            }
            ...
            let descent = (
                page.footer-descent.ratio * bottomm + page.footer-descent.length
            )
            place(bottom, dy: -bottomm, line(length: 100%, stroke: stroke))
            if footer {
                place(bottom, dy: -bottomm + descent, line(
                    length: 100%,
                    stroke: stroke,
                ))
            }

            place(left, dx: leftm.far, rect(
                width: leftm.width,
                height: 100%,
                stroke: (x: stroke),
            ))
            place(left, dx: leftm.far + leftm.width + leftm.sep, line(
                length: 100%,
                stroke: stroke,
                angle: 90deg,
            ))

            place(right, dx: -rightm.far, rect(
                width: rightm.width,
                height: 100%,
                stroke: (x: stroke),
            ))
            place(right, dx: -rightm.far - rightm.width - rightm.sep, line(
                length: 100%,
                stroke: stroke,
                angle: 90deg,
            ))
    ...
    for (key, position) in positions {
        ...
        if cur <= position {
            ...
            if (
                ...
                empty = 0pt
                let new_x = calc.max(
                    position + items.at(key).height + clearance,
                    cur,
                )
                cur = new_x
...
#let place-note(
    ...
    box(
        ...
        context {
            ...
            let width = if side == "left" { get-left().width } else {
                get-right().width
            }
            ...
            let extends = if side == "right" { _note_extends_right } else {
                _note_extends_left
            }
...
#let note(
    ...
    let flush-numbering = if flush-numbering == auto {
        anchor-numbering != auto
    } else { flush-numbering }
    let anchor-numbering = if anchor-numbering == auto { numbering } else {
        anchor-numbering
    }

    // let keep-order = if keep-order == auto { not numbered } else { keep-orders }
    let shift = if shift == auto {
        if numbering != none { true } else { "avoid" }
    } else { shift }
    ...
    context {
        let side = if side == "outer" or side == auto {
            if _config.get().book and calc.even(here().page()) { "left" } else {
                "right"
            }
        } else if side == "inner" {
...
#let ref(
    ...
    if offset > 0 {
        ...
        assert(
            dest.len() > offset,
            message: "Not enough notes after this to reference",
        )
        ...
        assert(
            dest.len() >= -offset,
            message: "Not enough notes before this to reference",
        )
...
#let notefigure(
    ...
    context {
        ...
        let index = query(
            selector(<_marginalia_notefigure>).before(here()),
        ).len()
